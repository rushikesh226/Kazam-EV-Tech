...
const Cards = ({ home, setInputDiv, data, setUpdatedData }) => {
  ...
  const handleCompleteTask = async (id) => {
    try {
      await axios.put(
        `https://kazam-ev-tech-3.onrender.com/api/v2/update-complete-task/${id}`,
        {},
        { headers }
      );
    } catch (error) {
  ...
  const handleImportantTask = async (id) => {
    try {
      await axios.put(
        `https://kazam-ev-tech-3.onrender.com/api/v2/update-imp-task/${id}`,
        {},
        { headers }
      );
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      const resp = await axios.delete(
        `https://kazam-ev-tech-3.onrender.com/api/v2/delete-task/${id}`,
        { headers }
      );
  ...
  return (
    <div className="grid grid-cols-3 gap-4 p-4">
        ...
        return (
          ...
          >
            ...
            <div className="mt-4  w-full flex items-center">
              <button
                className={`${
                  card.complete === false ? "bg-red-400" : "bg-green-400"
                } p-2 rounded w-3/6`}
                onClick={() => handleCompleteTask(card._id)}
              >
                {card.complete ? "Completed" : "Incomplete"}
              </button>
              <div className=" p-2 w-3/6 text-2xl flex justify-around">
                <button onClick={() => handleImportantTask(card._id)}>
                  {card.important ? (
                ...
                {home !== "false" && (
                  <button
                    onClick={() =>
                      handleEditTask(card._id, card.title, card.description)
                    }
                  >
                    <FaEdit />
                  </button>
      ...
      {home === "true" && (
        <div className="bg-gray-800 rounded-sm p-4 flex flex-col justify-center items-center text-gray-300 hover:scale-105 hover:cursor-pointer transition-all duration-300">
